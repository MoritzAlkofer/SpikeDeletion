...
def write_to_results(results,group_name,N_patients,N_recording, N_female,N_male, N_samples, N_spikes, N_Routine, N_ltm, N_emu, N_Other):
        ...
        results['Other unit'].append(f'{N_Other} ({get_frac(N_Other,N_recording)})')
        return results

def build_table2(df,buckets,spike_threshold):
    results = {'Group name':[],'Patients':[],'Recordings':[],'Female':[],
               'Samples':[],'Spikes':[],'Routine':[], 'LTM':[], 'EMU':[], 'Other unit':[]}
...
def build_table2_clemson(df,spike_threshold):
    results = {'Group name':[],'Patients':[],'Recordings':[],'Female':[],
               'Samples':[],'Spikes':[],'Routine':[], 'LTM':[], 'EMU':[], 'Other unit':[]}
    group_name ='clemson'
    N_patients, N_recording, N_female, N_male, N_samples, N_spikes, N_Routine, N_ltm, N_emu, N_Other = get_info(df,spike_threshold)
...
if __name__=='__main__':
    ...
    table2s = []
    # build table 2 for all datasets
    for dataset in ['center','member','control']:
        print(dataset)
        df = load_dataset(dataset)
        df = df.merge(df_info,how='left',on='BonoboIndex')
        table2 = build_table2(df,buckets,spike_threshold)
        table2['dataset']=dataset
        table2s.append(table2)
    
    # do it for clemson
    df = load_dataset('external')
    df[['Sex','ServiceName']] = None,None
    table2 = build_table2_clemson(df,spike_threshold)
    table2['dataset']='external'
    table2s.append(table2)

    table2 = pd.concat(table2s)

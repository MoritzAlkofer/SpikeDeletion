...
def dataset_center_local():
    # returns high confidence dataframe, path and storage channels
    # Load Bonobo dataframe and add 'dataset' column
    path_df = '../Data/tables/event_split_localized_08Feb24.csv'
    df = pd.read_csv(path_df)
...
def dataset_member():
    ...
    df['patient_id']=df.eeg_file.str.split('_').str[0]
    df['dataset'] = 'member' 
    p = [0.9, 0.1] 
...
def assign_splits(df_center,df_member,df_control):
    df_control['patient_id']=df_control.eeg_file.str.split('_').str[0]
    df_member['Mode']=None
    for mode in ['Train','Val']:
        df_member.loc[df_member.patient_id.isin(df_center[df_center.Mode==mode].patient_id),'Mode']=mode
    df_control['Mode']=None
    for mode in ['Train','Val']:
        df_control.loc[df_control.patient_id.isin(df_center[df_center.Mode==mode].patient_id),'Mode']=mode
    return df_center, df_member, df_control

def prepare_df_and_metadata(datasets):
    ...
    df_center,df_member,df_control = assign_splits(metadata['center']['df'],metadata['member']['df'],metadata['control']['df'])
    df = pd.concat([df_center,df_member,df_control])    
...
def get_args():
    ...
    parser.add_argument('--random_channels',default=False, action= 'store_true')
    parser.add_argument('--split',default='representative', choices=['representative','localized'])
    args = parser.parse_args()
    return args.model_name, args.channels, args.split, args.random_channels
...
def init_datamodule_info(split):
    if split == 'representative':
        datasets = {'center':dataset_center_representative,'member':dataset_member,'control':dataset_control}
    if split == 'local':
        datasets = {'center':dataset_center_local,'member':dataset_member,'control':dataset_control}
    df,metadata = prepare_df_and_metadata(datasets)
    storage_channels = all_referential
    return df, metadata, storage_channels

def build_datamodule(df, metadata, storage_channels,random_channels=False):
    if random_channels:
        n_keeper_channels='random',
    ...
    datamodule = DeletedChannelsDatamoduleWithMembers(df=df,
                                                  ...
                                                  n_keeper_channels=n_keeper_channels,
                                                  keeper_channels=keeper_channels)
    return datamodule
...
if __name__ == '__main__':
    ...
    model_name, channels,split,random_channels = get_args()
    path_model = f'../Models/{model_name}'
    config = Config()
    config.BATCH_SIZE = 128
    config.CHANNELS = str_to_channel_list(channels)
    config.save_config(path_model)
    model_name = 'specialized_all_bipolar'
    df, metadata, storage_channels = init_datamodule_info(split)
    print("REMOVE THIS!!!")
    df = df[df.dataset=='center']

...
from utils import all_referential, KeepNRandomChannels, KeepFixedChannels
import numpy as np
import os
import pickle
import pandas as pd
...
if __name__=='__main__':
   ...
   n_runs = 1
   results = {'n_keeper':[],'run':[],'AUROC':[],'channel':[]}
   n_keeper = 1
   for channel in all_referential:
      for run in range(n_runs):
         #channel_remover = KeepNRandomChannels(len(config['CHANNELS']),N_keeper=n_keeper)
         channel_remover= KeepFixedChannels(config['CHANNELS'],[channel])
         ...
         print(f'channel {channel}, auc {roc_auc}')
   ...
   results.to_csv(os.path.join(path_model,'results_deleted_trash!.csv'),index=False)

...
if __name__=='__main__':
    ...
    df_thresh = pd.read_csv("Subprojects/find_thresholds/poi_thresholds.csv")
    results = {'Channels':[],'Accuracy':[],'Sensitivity':[],'Specificity':[],'Precision':[],'AUROC':[],'Threshold':[]}
    for i,ChannelLocation in enumerate(points_of_interest.keys()):
        ...
        results['AUROC'].append(auroc.round(2))
        threshold = df_thresh[df_thresh.channels==ChannelLocation].iloc[0].threshold
        y_score = binarize(y_score,threshold=threshold)
        ...
        results['Threshold'].append(threshold.round(2))
    ...
    for metric in ['Accuracy','Sensitivity','Specificity','Precision']:
        percentage = results[metric]/results[metric].iloc[-1]
        percentage = np.round(percentage,2)
        results[metric] = results[metric].astype(str) + ' ('+percentage.astype(str)+')'

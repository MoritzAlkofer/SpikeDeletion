from utils import all_referential, build_montage, normalize, cut_and_jitter
...
def load_model_from_checkpoint(path_model,config,architecture):
   if architecture =='Transformer':
      ...
      model = SpikeNetInstance.load_from_checkpoint(os.path.join(path_model,'weights.ckpt'),n_channels=19,map_location=torch.device('cuda'))                        
   return model

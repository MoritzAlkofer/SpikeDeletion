...
def plot_pr_curve(AUC,precision,recall):
    fig = plt.figure()
    # workaround for tresholds are too high
    plt.plot(recall,precision,label='PR curve (AUC = %0.3f)' % AUC)
...
def generate_metrics_subtitle(accuracy, sensitivity, specificity, precision):
    subtitle = 'accuracy: '+str(accuracy)+'\nsensitivity: '+str(sensitivity.round(2))+'\nspecificity: '+str(specificity)+'\nprecision: '+str(precision)
    return subtitle
...
def plot_confusion_matrix(cm):
    ...
    subtitle = generate_metrics_subtitle(accuracy, sensitivity, specificity, precision)

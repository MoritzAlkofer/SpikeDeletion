...
def increase_amp(signal):
   print('fincreasing signal size by factor 2!')
   return 10*signal
def get_transforms(montage_channels,storage_channels,windowsize,windowjitter,Fq):
   ...
   transforms = [montage,cutter,increase_amp]
...
if __name__=='__main__':
   ...
   df['preds']=preds
   df.to_csv(path_model+'/pred_clemson.csv',index=False)